...
def batch_normalization_layer(input_layer, is_training):
  ...
  bn_layer = tf.layers.batch_normalization(
         inputs=input_layer, axis = 3, momentum=0.997, epsilon=BN_EPSILON, center=True,
...
def inference(input_tensor_batch, n, is_training, reuse):
    ...
    with tf.variable_scope('conv0', reuse=reuse):
        conv0 = conv_bn_relu_layer(input_tensor_batch, [3, 3, 3, 16], 1, is_training)
    ...
    for i in range(n):
        with tf.variable_scope('conv1_%d' %i, reuse=reuse):
            if i == 0:
                conv1 = residual_block(layers[-1], 16, is_training, first_block=True)
            else:
                conv1 = residual_block(layers[-1], 16, is_training)
            activation_summary(conv1)
            layers.append(conv1)

    for i in range(n):
        with tf.variable_scope('conv2_%d' %i, reuse=reuse):
            conv2 = residual_block(layers[-1], 32, is_training)
            activation_summary(conv2)
            layers.append(conv2)

    for i in range(n):
        with tf.variable_scope('conv3_%d' %i, reuse=reuse):
            conv3 = residual_block(layers[-1], 64, is_training)
            layers.append(conv3)
...
def test_graph(train_dir='logs'):
    ...
    result = inference(input_tensor, 2, is_training = True, reuse=False)
