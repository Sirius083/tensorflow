...
def _read_one_batch(path, is_random_label):
    ...
    # dicts：
    # len(dicts) = 4
    # batch_label = str(size = 1)
    # data: (10000,3072) unit8
    # filenames: 10000, list
    # labels: 10000,list
    dicts = pickle.load(fo,encoding='latin1') # sirius 不加latin1的话会报错 'ascii' codec can't decode byte 0x8b in position 6: ordinal not in range(128)
    fo.close()
...
def read_in_all_images(address_list, shuffle=True, is_random_label = False):
    ...
    """
    data = np.array([]).reshape([0, IMG_WIDTH * IMG_HEIGHT * IMG_DEPTH]) # 只有一个维度为零时才可以这样初始化
    label = np.array([])
...
def whitening_image(image_np):
    ...
    图像的白化操作是对单张图像进行的
    '''
    for i in range(len(image_np)): # sirius: 多维tensor, len(array): 指的是第一个维度
        mean = np.mean(image_np[i, ...])
        # Use adjusted standard deviation here, in case the std == 0.
        std = np.max([np.std(image_np[i, ...]), 1.0/np.sqrt(IMG_HEIGHT * IMG_WIDTH * IMG_DEPTH)])  # sirius: ??
