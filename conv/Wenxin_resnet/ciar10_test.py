...
class Train(object):
    ...
    def build_train_validation_graph(self):
        ...
        # sirius: 是由于train/validation 定义了两组logits所以tensorboard中有两组operations的变量吗？并且两组变量还用tensor相连
        # sirius:???
        ...
        # validation data share all the weights with train data. 
        # This is implemented by passing reuse=True to the variable scopes of train graph
        logits = inference(self.image_placeholder, FLAGS.num_residual_blocks, is_training = True, reuse=False)
        vali_logits = inference(self.vali_image_placeholder, FLAGS.num_residual_blocks, is_training = True, reuse=True)
        ...
        self.full_loss = tf.add_n([loss] + regu_losses) # sitius: add elementwise
    ...
    def train(self):
        ...
        # cifar56层报错: failed to create cublas handle: CUBLAS_STATUS_ALLOC_FAILED
        config = tf.ConfigProto()
        config.gpu_options.allow_growth = True
        sess = tf.Session(config=config)
            
        # If you want to load from a checkpoint
        # sirius: retrain from a checkpoint file
        ...
        for step in range(FLAGS.train_steps): # 64000
            ...
            if step % FLAGS.report_freq == 0:
                ...
                if FLAGS.is_full_validation is True:
                    ...
                    summary_writer.flush() # sirius adding pending event to the disk
    ...
    def test(self, test_image_array):
        ...
        logits = inference(self.test_image_placeholder, FLAGS.num_residual_blocks, is_training = False, reuse=False)
        predictions = tf.nn.softmax(logits)
        
        # version 1: 从训练好的网络中加载
        ...
        saver.restore(sess, tf.train.latest_checkpoint(FLAGS.test_ckpt_dir))
        # saver.restore(sess, FLAGS.test_ckpt_path)
        print ('Model restored from ', tf.train.latest_checkpoint(FLAGS.test_ckpt_dir))
        
        '''
        # Version 2: 新版训练好的网络
        saver = tf.train.import_meta_graph(FLAGS.test_ckpt_path)
        ...
        if remain_images != 0:
            self.test_image_placeholder = tf.placeholder(dtype=tf.float32, shape=[remain_images,
                                                        IMG_HEIGHT, IMG_WIDTH, IMG_DEPTH])
            # Build the test graph
            logits = inference(self.test_image_placeholder, FLAGS.num_residual_blocks, is_training = False, reuse=True)
            predictions = tf.nn.softmax(logits)
            # sirius: ellipsis: a[...,0] = a[:,:,0] 
    ...
    def top_k_error(self, predictions, labels, k):
        ...
        if type(predictions) is np.ndarray: # 测试:nd_array
    ...
    def train_operation(self, global_step, total_loss, top1_error):
        ...
        ema = tf.train.ExponentialMovingAverage(FLAGS.train_ema_decay, global_step) # sirius: 0.95
    ...
    def full_validation(self, loss, top1_error, session, vali_data, vali_labels, batch_data,
        ...
        :return: float, float
        '''
        num_batches = 10000 // FLAGS.validation_batch_size # 250
        order = np.random.choice(10000, num_batches * FLAGS.validation_batch_size)
        vali_data_subset = vali_data[order, ...] # 打乱重新排序
...
#=============================================
# test
# 需要在hyper_parameters中改 model_ckpt_path,最新保存的model 
maybe_download_and_extract()
train = Train()
test_path = [r'E:/TF/ShareWeight/cifar10/cifar10_data/cifar-10-batches-py/test_batch'] # 必须是list的形式
test_data = read_in_all_images(test_path, shuffle=True, is_random_label = False)
test_labels = test_data[1]
# test_image_array[0]: (10000,32,32,3)
# test_image_array[1]: (10000,)
predictions = train.test(test_data[0])
test_top_1_error = train.top_k_error(predictions, test_labels, 1)
with tf.Session() as sess:
    test_error = sess.run(test_top_1_error)
print('test top 1 error', test_error) # 0.8892
